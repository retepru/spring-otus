...
public class BooksCommands {
    ...
    public String  create(@ShellOption({"name", "authorId", "styleId"}) String name, String authorId, Long styleId) {
        return bookService.create(name, authorId, styleId); // todo где правильно сделать проверку?
    }

    @ShellMethod(value = "Обновить книгу", key = {"b u", "book update"})
    public void update(@ShellOption({"id", "name", "authorId", "styleId"}) Long id, String name, String authorId, Long styleId) {
